...
const placesAutoComplete =  async (str)=>{
    const resultEn = await axios({
        method: 'get',
        url: `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${String(str)}&location=13.7451079,100.5320582&radius=30000&components=country:TH&language=en&key=${process.env.GOOGLE_API_KEY}`,
        headers: { }
      })
      const resultTh = await axios({
        method: 'get',
        url: `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${String(str)}&location=13.7451079,100.5320582&radius=30000&components=country:TH&language=th&key=${process.env.GOOGLE_API_KEY}`,
        headers: { }
      })
    return {
